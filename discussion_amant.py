...
with open('personnalite/amant_V.json') as file:
    data = json.load(file)
...
def check(cible_int):
    ...
    while True:
        ...
        if string.lower() == 'aide' :
            ...
            print("")
            print("""Intentions :   niveau 1//  presentation (p) ; connu (c) ; humeur (h) ;  aurevoir (exit) ; appeler (a) ; sujet (s) : engager (e) ; insulte (i) ;
                        grossier+ -------  ------- (g)
            - intimidation+  ------- (i)
            - violence+ --------- (v)
            - incompréhension+  --------- (inc)
            - compliment personne+ --------- (cp) (c)
            - compliment travail+ --------- (ct)
            - compliment idée+ --------- (ci)
            - rentre dedans+ (rd)
            - vérité+ (ve)
            - négation (n)
            ...
            Action amis
            -S'intéresser / écouter : -> raconte sa vie (sint)
            -S'inquiéter (sinq)
            -Clin d'oeil  ;)
            - Accolade  (acc)
            - question ?
        ...
        if string.lower() == 'aurevoir' :
            ...
            print("")
        elif string.lower() in ['saluer', "s'incliner", "s'intéresser", "s'inquiéter", "clin d'oeil"]:
            print("")
            ...
            if test_caractere > 50:
                ...
                destination.habitants[cible_int].__dict__['attirance'] = destination.habitants[cible_int].__dict__['attirance'] - 100
            print("")
        elif string.lower() in ['embrasser', 'enlacer', "toucher", "caresser", "prendre la main", "regarder", "pousser", "masser"]:
            print("")
            print("destination.habitants[cible_int].__dict__['personalite']")
            print(destination.habitants[cible_int].__dict__['personalite'])
            if destination.habitants[cible_int].__dict__['personalite'] == "empathique":
            ...
            if test_caractere > 50:
                print("Le geste semble avoir eu un effet positif ;)")
                destination.habitants[cible_int].__dict__['relationnel'] = destination.habitants[cible_int].__dict__['relationnel'] + 350
                destination.habitants[cible_int].__dict__['peur'] =  destination.habitants[cible_int].__dict__['peur'] - 350
                destination.habitants[cible_int].__dict__['attirance'] = destination.habitants[cible_int].__dict__['attirance'] + 500
            elif test_caractere < 50 and test_caractere > 25:
                print("Le geste semble ne pas avoir fait d'effet...")
            else:
                print("Le geste semble avoir eu un effet négatif !")
            ...
            for i in data['intents']:
                if i['tag']==category:
                    ...
                    if i['tag'] =="presentation":
                        ...
                        res = test_list1.insert(4, test_list2.pop(test_list2.index("chat")))
                    elif i['tag'] =="sujet":
            ...
            if attirance > 1000:
                ...
                destination.habitants[cible_int].__dict__['attitude'] = "amant"
        else:
